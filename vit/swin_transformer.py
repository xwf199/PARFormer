    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=20, in_chans=3, embed_dim=768):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.weight.data.fill_(1)
                m.bias.data.zero_()
        patches_resolution = [self.num_x, self.num_y]
        self.patches_resolution = patches_resolution
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, mask_ratio=0, **kwargs):
        super().__init__()
        ...
        # self.patch_embed = PatchEmbed_overlap(img_size=img_size, patch_size=4, stride_size=3, in_chans=in_chans, embed_dim=embed_dim)
        ...
        self.classifier = ClassBlock(768, 35, droprate=0.5, linear=512, return_f = False)

        ######
        self.brm = patch_mask(ratio= self.maskratio)
        ...
        self.head2 = nn.Linear(self.num_features, 3, bias=False)
        self.head2.apply(weights_init_classifier2)
    ...
    def forward(self, x, tid):
        dx = self.forward_features(x)
        if(tid ==1):
            dx = self.brm(dx)

        dx = self.norm(dx)
        dx = self.avgpool(dx.transpose(1, 2))
